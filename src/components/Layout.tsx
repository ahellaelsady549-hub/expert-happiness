...
const navItems = [
  ...
  { to: "/azkar", label: "الأذكار", icon: Heart },
...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <MenuFab onClick={() => setOpen(true)} hidden={open} />
    </div>
  );
}

function MenuFab({ onClick, hidden }: { onClick: () => void; hidden: boolean }) {
  return (
    <button
      onClick={onClick}
      aria-label="فتح القائمة"
      className={`fixed bottom-6 left-6 z-30 flex h-14 w-14 items-center justify-center rounded-full hero-gradient text-gold glow transition-all duration-300 lg:hidden ${
        hidden ? "scale-0 opacity-0" : "scale-100 opacity-100"
      }`}
    >
